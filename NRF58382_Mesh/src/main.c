...
{	
	...
	if (data == unlock_code || data == 0x4aa2333)
	{
		
	        printk("PRBS CODE GENERATED INSIDE IF STATMENT %x\n",unlock_code);
		printk("ITS A MATCH\n");
...
void main(void)
{
	...
	if (err < 0)
	{
		printk("Error configuring GPIO0 \n");
		while(1);
	}		
